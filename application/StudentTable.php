...
class StudentTable
{
    ...
    public function createTable($studentGrid, $allSubject, $num_subject)
    {
        ...
        foreach ($studentGrid as $student) {
            ...
            echo "<tr><td >{$student['STUDENT_ID']}</td>
                    <td>{$student['STUDENT_FNAME']} {$student['STUDENT_LNAME']}({$student['STUDENT_FNUMBER']})</td>
            ...
            for ($i = 0; $i < $num_subject; $i++) {
                ...
                $asses = $this->assessmentsToString($list_of_subject[$i]);
            ...
            $asses_to_string = $this->assessmentsToString($string_asess_avg);

            echo '<td>' . $asses_to_string . '(' . $string_asess_avg . ')</td>';
            echo "</tr>";
        }
    ...
    private function  assessmentsToString($assessment)
    {
        switch ($assessment) {
            case '2':
                $asses_string = 'Слаб';
