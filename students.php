...
<?php while ($row = $result->fetch_assoc()) {
    ...
    Специалност:<select name="stu-spec">
<?php $result = $mysqli->query('SELECT * FROM specialities ');
while ($row = $result->fetch_assoc()) {
    echo "<option value=" . $row['speciality_name_long'] . ">" . $row['speciality_name_long'] . "</option>";
} ?>  </select><br>
...
<table width="200" border="1">
   ...
   /**     if (isset($_GET["kurs"])&& !empty($_GET['kurs'])) {
        $query .= sprintf(" AND course_name='%s'",$_GET["kurs"]);
    }
        if (isset($_GET["stu-spec"])&& !empty($_GET['stu-spec'])) {
        $query .= sprintf(" AND subject_name='%s'",$_GET["stu-spec"]);
    }*/
